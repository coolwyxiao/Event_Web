	  ...
	  public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		for (String itemId : favoriteItemIds) {
			...
			for (String category : categories) {
				...
				// 如果之前有就直接覆盖并放进hashmap中
				//不能直接用get（category）++，因为会把值拿出来之后++然后返回副本，需要用put的方法（这里不能用++因为这里是Integer）
			}
		}
		
		//可以将key value pairs放进list中
		...
		// 由于这里需要使用set，需要判断两个值是否相等，所以需要写一个equals和hashcode（）在Item中
